...
class NaiveLSTM(nn.Module):
    ...
    def forward(self, inputs, state=None) :
        ...
        if state is None:
            h_t = torch.zeros(1, self.hidden_size).t()#(20,1)
        ...
        for t in range(seq_size):
            x = inputs[:, t, :].t()#torch.Size([10, 1])

            # input gate
            i = torch.sigmoid(self.w_ii @ x + self.b_ii + self.w_hi @ h_t +
                              self.b_hi)#torch.Size([20, 1])
            ...
            c_next = f * c_t + i * g #torch.Size([20, 1])
...
# output1, (hn1, cn1) = naive_lstm(inputs)
